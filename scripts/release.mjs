...
function bumpVersion(version)
{
    ...
    // Read first and treat "not there" as an outcome, rather than asking whether it exists
    // and then reading: between the two answers the file can change, which is the shape
    // CodeQL flags as a file-system race. Behaviour is identical - a missing lockfile is
    // still skipped silently - but there is no window between the question and the answer.
